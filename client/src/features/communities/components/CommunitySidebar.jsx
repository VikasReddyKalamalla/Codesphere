...
export const CommunitySidebar = ({ activeCommunityId, selectedCategory, onCategoryChange }) => {
  ...
  const handleJoinToggle = (e, communityId, isMember) => {
    e.stopPropagation();
    if (isMember) {
  ...
  return (
    <aside className="w-80 bg-white dark:bg-slate-900 border-r border-slate-150 dark:border-slate-800 flex flex-col select-none shrink-0 h-full text-left">
      
      {/* Sidebar Header - matches the height and bottom border of main content header */}
      <div className="bg-white dark:bg-slate-900 border-b border-slate-150 dark:border-slate-800 px-6 flex items-center shrink-0 h-24">
      ...
      <div className="flex-1 p-6 flex flex-col gap-6 overflow-y-auto no-scrollbar">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <h4 className="text-xs font-bold text-slate-850 dark:text-slate-100 uppercase tracking-wider font-mono">Discover Communities</h4>
            <button 
                ...
                <div 
                  ...
                  className={`flex items-center justify-between p-2.5 rounded-xl border transition-all cursor-pointer ${
                    isActive 
                      ...
                      : 'bg-slate-50 dark:bg-slate-950/30 border-transparent hover:border-slate-200 dark:hover:border-slate-800 hover:bg-slate-100/50 dark:hover:bg-slate-950/50'
                  }`}
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="w-8 h-8 rounded-lg overflow-hidden bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-855 flex items-center justify-center shrink-0">
                      {comm.logo ? (
                    ...
                    <div className="min-w-0">
                      <span className="text-xs font-bold text-slate-700 dark:text-slate-205 truncate block">{comm.name}</span>
                      <span className="text-[9px] font-mono text-slate-400 dark:text-slate-500">{comm.memberCount || 0} Members</span>
                    </div>
                  </div>
                  ...
                  <button 
                    ...
                    className={`text-[9px] font-bold px-3 py-1 rounded-lg border transition-all cursor-pointer ${
                      isMember 
                        ? 'text-slate-455 border-slate-200 dark:border-slate-800 hover:bg-red-50 dark:hover:bg-red-950/20 hover:text-red-600 hover:border-red-500/20' 
                        : 'hover:opacity-90'
                    }`}
                  >
        ...
        <div className="flex flex-col gap-3">
          <h4 className="text-xs font-bold text-slate-850 dark:text-slate-100 uppercase tracking-wider font-mono">Categories</h4>
              ...
              return (
                <button 
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all border ${
                    isActive 
                      ? 'bg-emerald-50 dark:bg-emerald-950/20 text-[#04AA6D] dark:text-emerald-400 border-emerald-100 dark:border-emerald-500/20 shadow-sm' 
                      : 'text-slate-550 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-950/40 border-transparent'
                  }`}
                >
                  <cat.icon size={13} className={isActive ? 'text-[#04AA6D] dark:text-emerald-400' : 'text-slate-455 dark:text-slate-500'} />
                  <span>{cat.label}</span>
                </button>
              );
            })}
        ...
        <div className="mt-auto bg-slate-50 dark:bg-slate-950 border border-slate-150 dark:border-slate-800 rounded-3xl p-5 relative overflow-hidden shadow-sm shrink-0">
          ...
          <h5 className="text-xs font-black text-slate-700 dark:text-slate-100 uppercase tracking-wider font-mono">Create Your Space</h5>
          ...
          <button 
            ...
            className="w-full mt-4 bg-[#04AA6D] hover:bg-[#03935e] text-white text-[10px] font-bold font-mono uppercase tracking-wider py-2.5 rounded-xl transition-all shadow-lg shadow-emerald-500/20 cursor-pointer flex items-center justify-center gap-1.5"
          >
            <PlusCircle size={12} />
