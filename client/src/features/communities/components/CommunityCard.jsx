...
export const CommunityCard = ({ community = {}, onJoinToggle, isJoined }) => {
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800 rounded-2xl p-5 hover:border-emerald-500/30 dark:hover:border-emerald-500/30 transition-all shadow-sm dark:shadow-xl text-left select-none flex flex-col justify-between h-full group">
      <div>
        {/* Banner placeholder */}
        <div className="h-16 w-full rounded-xl bg-gradient-to-r from-emerald-900/10 to-emerald-900/20 dark:from-emerald-900/30 dark:to-emerald-900/30 border border-slate-100 dark:border-slate-850 flex items-center justify-end p-2 relative overflow-hidden mb-4 animate-pulse">
        ...
        <div className="flex gap-3 items-start relative mt-[-32px] px-2 mb-3">
          <div className="w-10 h-10 rounded-xl overflow-hidden border border-slate-150 dark:border-slate-800 bg-white dark:bg-slate-950 flex items-center justify-center shrink-0 shadow-md">
            {community.logo ? (
      ...
      {/* Footer details */}
      <div className="flex justify-between items-center mt-5 pt-3 border-t border-slate-100 dark:border-slate-850">
        <span className="text-[9px] font-bold font-mono text-slate-455 dark:text-slate-500 uppercase tracking-wider">{community.memberCount || 0} Members</span>
        ...
        <button 
          ...
          className={`text-[10px] font-bold px-3.5 py-1.5 rounded-lg border transition-all cursor-pointer ${
            isJoined 
              ? 'text-slate-455 dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:bg-red-50 dark:hover:bg-red-950/20 hover:text-red-600 dark:hover:text-red-400 hover:border-red-500/20' 
