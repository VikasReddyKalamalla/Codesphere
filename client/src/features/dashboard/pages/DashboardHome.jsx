  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-4 gap-6 w-full text-left bg-slate-50 dark:bg-slate-950 transition-colors duration-200 font-sans pb-12 select-none">
      ...
      <div className="xl:col-span-3 flex flex-col gap-6">
        ...
        <div className="relative overflow-hidden bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-6 sm:p-8 text-slate-900 dark:text-white shadow-sm transition-colors duration-200">
          {/* Subtle Background Glows */}
          <div className="absolute top-0 right-0 w-80 h-80 bg-emerald-500/10 dark:bg-[#04AA6D]/15 rounded-full blur-[90px] pointer-events-none" />
          <div className="absolute bottom-0 left-1/3 w-60 h-60 bg-emerald-500/5 dark:bg-emerald-500/10 rounded-full blur-[80px] pointer-events-none" />

          <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-2.5 mb-2">
                <span className="px-3 py-1 rounded-full text-[10px] font-bold font-mono bg-emerald-50 dark:bg-[#04AA6D]/20 text-[#04AA6D] dark:text-emerald-400 border border-emerald-200/80 dark:border-[#04AA6D]/40 uppercase tracking-widest flex items-center gap-1.5">
                  <Sparkles size={12} />
                  <span>CS STUDENT PORTAL</span>
                </span>
                <span className="text-xs font-mono text-slate-500 dark:text-slate-400">• {currentDateStr}</span>
              </div>
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <Link
                to="/learning"
                className="flex items-center gap-2 px-5 py-3 bg-[#04AA6D] hover:bg-emerald-600 text-white text-xs font-mono font-bold rounded-2xl transition-all cursor-pointer shadow-md shadow-emerald-500/10 border border-emerald-500/30"
              >
                <BookOpen size={15} />
        ...
        <div className="relative overflow-hidden bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-6 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-6 text-slate-900 dark:text-white transition-colors duration-200">
          <div className="absolute top-0 right-0 w-60 h-60 bg-emerald-500/10 dark:bg-[#04AA6D]/15 rounded-full blur-[80px] pointer-events-none" />
          <div className="flex items-center gap-4 flex-1 relative z-10">
            <div className="w-14 h-14 rounded-2xl bg-emerald-50 dark:bg-[#04AA6D]/20 border border-emerald-200 dark:border-[#04AA6D]/40 text-[#04AA6D] flex items-center justify-center shrink-0 font-mono font-black text-xl shadow-xs">
              &lt;/&gt;
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-[#04AA6D] dark:text-emerald-400 bg-emerald-50 dark:bg-[#04AA6D]/20 px-2.5 py-0.5 rounded-full border border-emerald-200 dark:border-[#04AA6D]/40">
          ...
          <Link
            ...
            className="flex items-center gap-2 px-6 py-3 bg-[#04AA6D] hover:bg-emerald-600 text-white font-mono font-bold text-xs rounded-2xl transition-all shadow-md shrink-0 border border-emerald-400/30 relative z-10"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className={`${cardClass} flex flex-col justify-between`}>
            ...
            <Link to="/learning" className="mt-4 block w-full py-2.5 bg-emerald-50/60 dark:bg-emerald-950/40 hover:bg-emerald-100/80 dark:hover:bg-emerald-900/40 text-[#04AA6D] dark:text-emerald-400 border border-emerald-200/40 dark:border-emerald-800/40 rounded-2xl text-center text-xs font-bold font-mono uppercase tracking-wider transition-all">
              ...
              <div className="flex flex-col gap-3">
                {dbData?.continueLearning && dbData.continueLearning.length > 0 ? (
                  dbData.continueLearning.slice(0, 3).map((path) => (
                    <Link key={path.learningPathId} to={path.continueRoute} className="flex gap-3 items-center text-left hover:bg-slate-50 dark:hover:bg-slate-800/60 p-2 rounded-2xl transition-all border border-transparent hover:border-slate-200 dark:hover:border-slate-700">
                      <div className="shrink-0">
                        <div className="w-10 h-9 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-[10px] text-white font-bold font-mono shadow-xs select-none">
                          &lt;/&gt;
                        </div>
                      </div>
                      <div className="flex-1 min-w-0 flex flex-col gap-1">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs font-mono font-extrabold text-emerald-500">{path.completionPercentage || 0}%</span>
                        </div>
                  ...
                  <div className="flex flex-col items-center justify-center py-6 text-center">
                    ...
                    <Link to="/learning" className="text-[10px] text-[#04AA6D] hover:underline font-bold font-mono uppercase mt-1">Browse 83 courses</Link>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className={`${cardClass} flex flex-col justify-between`}>
                ...
                {dbData?.deadlines && dbData.deadlines.length > 0 ? (
                  dbData.deadlines.map((t) => (
                    <div key={t.title} className="flex items-center gap-2.5 p-2.5 rounded-2xl bg-slate-50/60 dark:bg-slate-950/60 border border-slate-100 dark:border-slate-800">
                      <div className="w-7 h-7 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-100/50 dark:border-emerald-900/50 flex items-center justify-center shrink-0 text-[#04AA6D] dark:text-emerald-400">
                        <CheckSquare size={14} />
                      </div>
            ...
            <Link to="/tests" className="mt-4 block w-full py-2.5 bg-emerald-50/60 dark:bg-emerald-950/40 hover:bg-emerald-100/80 dark:hover:bg-emerald-900/40 text-[#04AA6D] dark:text-emerald-400 border border-emerald-200/40 dark:border-emerald-800/40 rounded-2xl text-center text-xs font-bold font-mono uppercase tracking-wider transition-all">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className="shrink-0 text-[8px] font-bold px-2 py-0.5 rounded-full uppercase bg-emerald-50 dark:bg-emerald-950/40 text-[#04AA6D] dark:text-emerald-400 border border-emerald-100/50 dark:border-emerald-900/50 font-mono">
                            {i.status}
                          </span>
                        </div>
                      </div>
                ...
                dbData.reminders.map((r) => {
                  ...
                  return (
                    <div key={r.title} className="flex items-center gap-2.5 p-2.5 rounded-2xl bg-slate-50/60 dark:bg-slate-950/60 border border-slate-100 dark:border-slate-800">
                      ...
                      <span className="shrink-0 text-[8.5px] font-bold text-[#04AA6D] dark:text-emerald-400 font-mono">
                        {r.time}
                      </span>
                    </div>
                  );
      ...
      <div className="xl:col-span-1 flex flex-col gap-6 animate-fade-in">
        ...
        <div className={`${cardClass} flex flex-col justify-between min-h-[380px]`}>
          ...
          <Link to="/notifications" className="mt-4 block w-full py-2.5 bg-emerald-50/60 dark:bg-emerald-950/40 hover:bg-emerald-100/80 dark:hover:bg-emerald-900/40 text-[#04AA6D] dark:text-emerald-400 border border-emerald-200/40 dark:border-emerald-800/40 rounded-2xl text-center text-xs font-bold font-mono uppercase tracking-wider transition-all">
        ...
        <div className={`${cardClass}`}>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              { label: 'Podcast',      icon: Video,          color: 'bg-emerald-50 dark:bg-emerald-950/40 border-emerald-100 dark:border-emerald-900/50 text-[#04AA6D] dark:text-emerald-400' },
