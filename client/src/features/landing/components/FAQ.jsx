    ...
    <section ref={ref} className="px-6 py-24 bg-transparent border-t border-slate-200 z-10 relative">
      <div className="max-w-2xl mx-auto">
        ...
        >
          ...
          <h2 className="font-mono-origin text-2xl sm:text-4xl font-bold text-slate-800 tracking-tight mt-3">
            Common Questions
          </h2>
        </motion.div>
        ...
        <motion.div
          ...
          className="rounded-2xl overflow-hidden border border-slate-200 bg-white/95 backdrop-blur-sm shadow-md"
        >
          {faqs.map((faq, i) => (
            <div key={i} className={i < faqs.length - 1 ? 'border-b border-slate-200' : ''}>
              <button
                onClick={() => setOpen(open === i ? null : i)}
                className="w-full flex items-center justify-between px-6 py-5 text-left transition-colors hover:bg-slate-50/50 group"
              >
                <motion.span 
                  whileHover={{ x: 4 }}
                  transition={{ type: 'spring', stiffness: 300, damping: 20 }}
                  className="font-mono-origin text-xs font-bold text-slate-800 group-hover:text-[#04AA6D] transition-colors"
                >
                  {faq.q}
                </motion.span>
                {open === i
                  ? <Minus className="w-4 h-4 shrink-0 text-slate-400 group-hover:text-[#04AA6D]" />
                  : <Plus  className="w-4 h-4 shrink-0 text-slate-400 group-hover:text-[#04AA6D]" />
                }
              </button>
              <AnimatePresence initial={false}>
                {open === i && (
                  ...
                  >
                    <div className="px-6 pb-5 font-sans-origin text-xs text-slate-500 leading-relaxed">
                      {faq.a}
                    </div>
                  </motion.div>
                )}
