        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <motion.div
              ...
              className="flex flex-col gap-5 p-6 rounded-xl border border-slate-200 bg-white/95 backdrop-blur-sm shadow-md text-left cursor-pointer transition-colors"
            >
              <p className="font-mono-origin text-xs text-slate-655 leading-relaxed flex-1">
                "{t.quote}"
              </p>
              <div className="flex items-center gap-3 pt-4 border-t border-slate-200 select-none">
                ...
                <div>
                  <p className="font-sans-origin text-xs font-bold text-slate-800 leading-tight">{t.name}</p>
                  <p className="font-mono-origin text-[9px] text-slate-500 mt-1 uppercase tracking-wider">{t.role}</p>
                </div>
              </div>
            </motion.div>
          ))}
